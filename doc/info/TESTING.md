...
4. **PYTHONPATH=src** is required for `python -m compileall`, `python src/main.py --smoke`, and subprocess tests. Pytest from the repo root already adds `src` via `[tool.pytest.ini_options] pythonpath` in [`pyproject.toml`](../../pyproject.toml); setting the env var locally still matches CI.
...
Existing: [`tests/test_logger_fallbacks.py`](../../tests/test_logger_fallbacks.py) (`test_gerber_gerbonara_fallback_logs_warning`, `test_hanwha_odbc_fallback_logs_warning`). Spy the logger with `pytest-mock`; do not replace it with a silent stub:
...
Existing pattern: [`src/app/workers.py`](../../src/app/workers.py), [`tests/test_workers_threads.py`](../../tests/test_workers_threads.py), [`tests/test_cross_check_thread.py`](../../tests/test_cross_check_thread.py), [`tests/test_machine_lib_open_reload.py`](../../tests/test_machine_lib_open_reload.py). House style is `QApplication.processEvents()` plus `thread.wait(…)` (see `_wait_thread`), then `deleteLater()`. `pytest-qt` is in `requirements-dev.txt` but **no test uses `qtbot` today**. If you add `qtbot.waitSignal(thread.result_ready, timeout=…)` around `thread.start()`, that is an allowed alternative — still no `qtbot` clicks inside `QThread.run()`.
...
Every `connect_mdb` needs `close()` in `finally` (see [`import_mdb_to_cache`](../../src/machine_library/hanwha_sqlite_cache.py)). Tests that mock `pyodbc.connect` / `connect_mdb` should assert `close` was called on the connection mock after the function returns (success **and** mid-loop failure). Existing: [`tests/test_access_odbc.py`](../../tests/test_access_odbc.py), [`tests/test_hanwha_save_odbc.py`](../../tests/test_hanwha_save_odbc.py) (`conn.close.assert_called_once()` after insert failure), [`tests/test_hanwha_sqlite_cache.py`](../../tests/test_hanwha_sqlite_cache.py) (`test_import_mdb_to_cache_closes_odbc_on_mid_loop_failure`). Do not leave pooling on (`pooling = False` in [`access_odbc.py`](../../src/machine_library/access_odbc.py)).
...
### Mocking: `monkeypatch` vs `pytest-mock`

- **`monkeypatch.setenv` / `delenv`:** env flags (`VALVET_DEBUG`, `BOOMER_MISSING_TOKENS_LOG`, `QT_QPA_PLATFORM`).
- **`monkeypatch.setattr`:** replace a function or module attribute for the duration of one test (backends, `read_pickle`, `QMessageBox`).
- **`mocker.spy` / `mocker.patch`:** assert call counts and mock objects (`logger.warning`, `conn.close`). Requires `pytest-mock` from `requirements-dev.txt` (CI installs it; a bare `requirements.txt` venv will error with `fixture 'mocker' not found`).
...
```bash
python -m pytest tests -q --ignore=tests/test_pcb_preview_gerber.py -m "not slow"
```

There is **no** `@pytest.mark.smoke`. App smoke is `python src/main.py --smoke` (CI step), not a pytest marker.
...
| `pytest.importorskip("PySide6")`              | Many Qt tests; `tests/test_qsettings_bom_pnp_persist.py`, `tests/test_working_copy_ui.py`; **one test** in `tests/test_hanwha_column_labels_and_filters.py` and `test_step_3d.py` | Minimal env **without** PySide6: those skip; full `requirements.txt` includes PySide6 — CI should install full requirements. |
...
| Access ODBC (no timeout / close)     | `tests/test_access_odbc.py`, `tests/test_hanwha_save_odbc.py` (`close` on save failure)                             |
| Workers / QThread                    | `tests/test_workers_threads.py`                                                                                   |
| Core has no PySide6 (CI step)        | `tests/test_no_pyside6_core.py`, `tests/test_cli_no_qt.py`                                                        |
| `src/main.py --smoke` subprocess     | `tests/test_main_smoke.py`                                                                                        |
...
| Working-copy / autosave pickle       | `tests/test_working_copy.py`, `tests/test_working_copy_ui.py`                                                      |
...
Full discovery (every `test_*.py`, not only this table): `python -m pytest tests --collect-only -q`.
...
```bash
python -m pytest tests -x --pdb -s          # stop at first failure, debugger
python -m pytest tests --durations=10 -q    # ten slowest tests
python -m pytest tests -m slow -q           # live UPD.MDB import only
python -X dev -m pytest tests/test_app_startup.py -s   # extra checks; Qt/C++ aborts still need a debugger
```

`python -X dev` helps with ResourceWarnings and some interpreter checks. It does **not** turn a PySide6 native crash into a Python traceback by itself.
...
- **Invariant gaps:** natsort dirty-input fixture (when natural sort is added); missing-file / corrupt import tests for BOM/PnP/Gerber beyond happy path. ODBC `close()` on Hanwha save failure is covered by [`tests/test_hanwha_save_odbc.py`](../../tests/test_hanwha_save_odbc.py).
