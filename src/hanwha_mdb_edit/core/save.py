...
def save_enriched_library(
    mdb_path: str | Path, enriched_df: pd.DataFrame
) -> SaveResult:
    """
    ...
    prof_merged = _merge_snapshot(
        prof_full,
        patches["PROFILE_Det"],
        "PROFILENAME",
        ["PARENTPROFILE", "UPDPARTGROUPID"],
    )

    com_full = load_table_dataframe(mdb_p, "PROFILECOMDATA_Det")
    com_merged = _merge_snapshot(
        com_full, patches["PROFILECOMDATA"], "PROFILENAME", ["FEEDINGSPEEDLEVEL"]
    )

    qh_full = load_table_dataframe(mdb_p, "Q_HANDDATA_Det")
    qh_merged = _merge_snapshot(
        qh_full, patches["Q_HANDDATA"], "PROFILENAME", ["OVERALL_SPEED_LEVEL"]
    )
...
def save_part_det(mdb_path: str | Path, df: pd.DataFrame) -> SaveResult:
    ...
    if sys.platform == "win32":
        try:
            _save_part_pyodbc(mdb_p, part_only)
            return SaveResult(
                backup_path=bak, mode="mdb_pyodbc", exported_paths=(p_part,)
            )
        except HanwhaSaveError as e:
