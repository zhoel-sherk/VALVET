...
class EditablePartDetRow:
    ...
    def validate(self) -> None:
        if len(self.partname) > MAX_PARTNAME_LEN:
            raise HanwhaValidationError(
                f"PARTNAME exceeds {MAX_PARTNAME_LEN} characters."
            )
        if len(self.profilename) > MAX_PROFILENAME_LEN:
            raise HanwhaValidationError(
                f"PROFILENAME exceeds {MAX_PROFILENAME_LEN} characters."
            )
        if len(self.partdesc) > MAX_PARTDESC_LEN:
            raise HanwhaValidationError(
                f"PARTDESC exceeds {MAX_PARTDESC_LEN} characters."
            )
