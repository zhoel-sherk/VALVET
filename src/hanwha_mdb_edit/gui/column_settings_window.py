...
class HanwhaMdbColumnSettingsWindow(QtWidgets.QWidget):
    ...
    def __init__(
        self, editor: HanwhaMdbEditorWindow, *, parent: QtWidgets.QWidget | None = None
    ):
        ...
        b_hide_merged.setToolTip(
            "Uncheck columns whose names contain '__' (joined extra tables)."
        )
    ...
    def rebuild_from_editor(self) -> None:
        ...
        for col in cols:
            it = QtWidgets.QListWidgetItem(
                self._editor.format_column_for_config_list(col)
            )
