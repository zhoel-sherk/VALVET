...
class HanwhaMdbEditorWindow(QtWidgets.QMainWindow):
    ...
    def __init__(
        ...
        *,
        ...
        _fs.endGroup()
        self._chk_hide_standard_s.setChecked(
    ...
    def apply_column_visibility(
        self, hidden: set[str], *, persist: bool = True
    ) -> None:
    ...
    def _open_part_detail(self) -> None:
        ...
        if row is None:
            QtWidgets.QMessageBox.information(
                self, "Component editor", "Select a row in the table."
            )
            return
    ...
    def _reload(self, *, force_original: bool = False) -> None:
        ...
        recovered = (
            None
            if force_original
            else prompt_recover_snapshot(
                self, path, "hanwha_mdb", self._hanwha_autosave_dir
            )
        )
    ...
    def _notify_saved(self, result: SaveResult) -> None:
        ...
        if result.mode == "mdb_pyodbc":
            msg = (
                "Library tables updated via ODBC (PART + profiles).\n\n"
                f"Backup:\n{backup}\n\nCSV snapshots:\n{paths}"
            )
        else:
            msg = (
    ...
    def _bulk_feed_speed(self) -> None:
        ...
        if "PARENTPROFILE" in df.columns:
            paren.addItems(
                sorted({str(x) for x in df["PARENTPROFILE"].dropna().unique()})
            )
