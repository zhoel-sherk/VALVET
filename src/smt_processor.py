...
def read_file(
    ...
    *,
    ...
    if suffix in [".xlsx", ".xls"]:
        df = _read_excel(
            path, sheet_name, column_headers_from_file=column_headers_from_file
        )
        ...
        if separator and separator != "auto":
            if separator == "2+sp":
                ...
                sep_value = separator
        df = _read_csv(
            path, separator=sep_value, column_headers_from_file=column_headers_from_file
        )
    elif suffix == ".ods":
        df = _read_ods(
            path, sheet_name, column_headers_from_file=column_headers_from_file
...
def apply_row_as_column_header(df: pd.DataFrame, row_index: int) -> pd.DataFrame:
    ...
    when first_row points to the header line (same 1-based / 0-based convention as the GUI row range).
    """
    ...
    *,
    ...
    if separator in ("2+sp", "fixed"):
        return _read_fixed_width(
            path, 0
        )  # Use read_file(..., first_row=) to skip Board/header lines
...
def _read_fixed_width(path: str, start_row: int) -> pd.DataFrame:
    ...
    try:
        ...
        if is_xy_list and max_cols <= 6:
            ...
            cols = generic_11 + [
                f"Col{i + 1}" for i in range(len(generic_11), max_cols)
            ]
            actual_cols = max_cols
...
def clean_empty_rows(df: pd.DataFrame) -> pd.DataFrame:
    """Public alias: drop all-empty rows and Excel ``___`` separator rows."""
    return _clean_empty_rows(df)


def read_pnp_whitespace(
    path: str,
    *,
    first_row: int = 0,
    last_row: int = -1,
...
class SMTDataProcessor:
    ...
    def set_dataframes(
        ...
        bom_df: pd.DataFrame,
        pnp_df: pd.DataFrame,
        bom_config: ColumnConfig,
        pnp_config: ColumnConfig,
    ) -> "SMTDataProcessor":
        """
    ...
    def find_column_index(
        self, df: pd.DataFrame, col_identifier: str, has_header: bool = True
    ) -> int:
        """
    ...
    def cross_check(self) -> pd.DataFrame:
        ...
        if self._bom_df is None:
            raise SMTEmptyDataError(
                "BOM not loaded. Call load_bom() or set_dataframes() first."
            )
        if self._pnp_df is None:
            raise SMTEmptyDataError(
                "PnP not loaded. Call load_pnp() or set_dataframes() first."
            )
        ...
        try:
            bom_designator_idx = self.find_column_index(
                self._bom_df, bom_config.designator, bom_config.has_header
            )
            bom_comment_idx = self.find_column_index(
                self._bom_df, bom_config.comment, bom_config.has_header
            )
        except SMTColumnNotFoundError as e:
            raise SMTColumnNotFoundError(f"BOM configuration error: {e}")

        try:
            pnp_designator_idx = self.find_column_index(
                self._pnp_df, pnp_config.designator, pnp_config.has_header
            )
            pnp_comment_idx = self.find_column_index(
                self._pnp_df, pnp_config.comment, pnp_config.has_header
            )
        except SMTColumnNotFoundError as e:
        ...
        pnp_parts = self._extract_pnp_parts(
            pnp_designator_idx, pnp_comment_idx, pnp_config
        )
        ...
        for designator in bom_parts:
            if designator in pnp_parts:
                ...
                if bom_val != pnp_val:
                    footprint = (
                        pnp_parts[designator][1]
                        if len(pnp_parts[designator]) > 1
                        else None
                    )
                    cx = (
                        pnp_parts[designator][2]
                        if len(pnp_parts[designator]) > 2
                        else None
                    )
                    cy = (
                        pnp_parts[designator][3]
                        if len(pnp_parts[designator]) > 3
                        else None
                    )
                    results.append(
                        CrossCheckResult(
        ...
        if dup_pairs:
            self.config.emit_progress(
                f"Duplicate exact coordinates: {dup_pairs} pair(s)", "info"
            )

        # 5. Overlapping (close centers); optional O(n²) when check_overlap is enabled
        if self.config.check_overlap and self.config.min_distance_mm > 0:
            ...
            conflicts = _check_overlapping(
                pnp_parts, self.config.min_distance_mm, self.config.overlap_xy_are_mm
            )
            self.config.emit_progress(
                f"Overlap: {len(conflicts)} pair(s) within threshold", "info"
            )
    ...
    def _extract_bom_parts(
        self, designator_idx: int, comment_idx: int
    ) -> dict[str, str]:
        ...
        for _, row in self._bom_df.iterrows():
            ...
            comment = (
                str(row[comment_col])
                if comment_col and not pd.isna(row[comment_col])
                else ""
            )
    ...
    def _extract_pnp_parts(
        self, designator_idx: int, comment_idx: int, config: ColumnConfig
    ) -> dict[str, tuple]:
        ...
        for c in cols:
            ...
            if (
                config.coord_x not in ("?", "_skip_", None)
                and c_upper == str(config.coord_x).strip().upper()
            ):
                coord_x_col = c
            if (
                config.coord_y not in ("?", "_skip_", None)
                and c_upper == str(config.coord_y).strip().upper()
            ):
                coord_y_col = c
            if (
                config.footprint not in ("?", "_skip_", None)
                and c_upper == str(config.footprint).strip().upper()
            ):
                footprint_col = c
            if (
                config.layer not in ("?", "_skip_", None)
                and c_upper == str(config.layer).strip().upper()
            ):
                layer_col = c
    ...
    def merge_bom_pnp(self, include_dnp: bool = True) -> pd.DataFrame:
        ...
        for c in bom_cols:
            c_upper = str(c).strip().upper()
            if (
                bom_config.designator != "?"
                and c_upper == str(bom_config.designator).strip().upper()
            ):
                bom_designator_col = c
            if (
                bom_config.comment != "?"
                and c_upper == str(bom_config.comment).strip().upper()
            ):
                bom_comment_col = c
        ...
        for c in pnp_cols:
            c_upper = str(c).strip().upper()
            if (
                pnp_config.designator != "?"
                and c_upper == str(pnp_config.designator).strip().upper()
            ):
                pnp_designator_col = c
            if (
                pnp_config.comment != "?"
                and c_upper == str(pnp_config.comment).strip().upper()
            ):
                pnp_comment_col = c
        ...
        for c in pnp_cols:
            ...
            if (
                pnp_config.coord_x != "?"
                and c_upper == str(pnp_config.coord_x).strip().upper()
            ):
                pnp_x_col = c
            if (
                pnp_config.coord_y != "?"
                and c_upper == str(pnp_config.coord_y).strip().upper()
            ):
                pnp_y_col = c
            if (
                pnp_config.rotation != "?"
                and c_upper == str(pnp_config.rotation).strip().upper()
            ):
                pnp_rot_col = c
            if (
                pnp_config.layer != "?"
                and c_upper == str(pnp_config.layer).strip().upper()
            ):
                pnp_layer_col = c
            if (
                pnp_config.footprint != "?"
                and c_upper == str(pnp_config.footprint).strip().upper()
            ):
                pnp_fp_col = c
        ...
        for _, row in self._bom_df.iterrows():
            ...
            comment = (
                str(row[bom_comment_col])
                if bom_comment_col and not pd.isna(row[bom_comment_col])
                else ""
            )
            for d in designators:
                d = d.strip()
        ...
        for _, row in self._pnp_df.iterrows():
            ...
            if not value:
                value = (
                    str(row[pnp_comment_col])
                    if pnp_comment_col and not pd.isna(row[pnp_comment_col])
                    else ""
                )

            # Skip DNP
            if not include_dnp and str(value).strip().upper() in [
                "DNP",
                "DNP_FROM_BOM",
            ]:
                continue
            ...
            x = (
                merge_coordinate_cell(row[pnp_x_col])
                if pnp_x_col and not pd.isna(row[pnp_x_col])
                else np.nan
            )
            y = (
                merge_coordinate_cell(row[pnp_y_col])
...
def _check_overlapping(
    pnp_parts: dict, min_distance_mm: float, overlap_xy_are_mm: bool
) -> list[tuple]:
    """
