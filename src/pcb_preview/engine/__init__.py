...
def load_gerber_layer(path: str) -> GerberSvgPayload:
    ...
    if not Path(stripped).is_file():
        msg = f"Gerber file not found: {stripped}"
        logger.error(msg)
        return GerberSvgPayload(
    ...
    pyg = load_via_pygerber(stripped)
    if pyg.svg:
        return pyg
    gbn = load_via_gerbonara(stripped)
    if gbn.svg:
    ...
    logger.error("Gerber load failed for %s: %s", stripped, err_text)
    return GerberSvgPayload(
        source_path=stripped,
        svg="",
        bbox_mm=gbn.bbox_mm,
