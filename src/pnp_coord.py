...
def convert_xy_mm_to_mil_row(x_raw: object, y_raw: object) -> tuple[str, str]:
    ...
    if xd is None or yd is None:
        return "", ""
    return format_four_fractional_digits(xd * MM_TO_MIL), format_four_fractional_digits(
        yd * MM_TO_MIL
    )
...
def convert_xy_mil_to_mm_row(x_raw: object, y_raw: object) -> tuple[str, str]:
    ...
    if xd is None or yd is None:
        return "", ""
    return format_four_fractional_digits(xd * MIL_TO_MM), format_four_fractional_digits(
        yd * MIL_TO_MM
    )
