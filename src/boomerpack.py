"""
.boomerpack — ZIP restore bundle (manifest + pickled DataFrames).

format_version in manifest allows future members (e.g. Gerber snapshots).
"""
...
class BoomerpackError(Exception):
    pass
...
def save_boomerpack(
    ...
    *,
    ...
    """Write a .boomerpack zip. At least one of bom_df / pnp_df / merge_df must be non-empty."""
    ...
    if not (has_bom or has_pnp or has_merge):
        raise BoomerpackError(
            "Nothing to save: BOM, PnP, and Merge tables are all empty."
        )
    p = Path(path)
    if not str(p).lower().endswith(".boomerpack"):
        p = p.with_suffix(".boomerpack")
    manifest: dict[str, Any] = {
        "format_version": BOOMERPACK_FORMAT_VERSION,
        "app": "VALVET",
        "members": {
...
def load_boomerpack(path: str | Path) -> dict[str, Any]:
    """
    Read .boomerpack. Returns keys: manifest, meta, bom_df, pnp_df, merge_df (optional Nones).
    """
    p = Path(path)
    if not p.is_file():
        raise BoomerpackError(f"Not a file: {p}")
    with zipfile.ZipFile(p, "r") as zf:
        names = set(zf.namelist())
        if MANIFEST_NAME not in names:
            raise BoomerpackError(f"Missing {MANIFEST_NAME}")
        manifest = json.loads(zf.read(MANIFEST_NAME).decode("utf-8"))
        fv = int(manifest.get("format_version", 0))
        if fv != BOOMERPACK_FORMAT_VERSION:
            raise BoomerpackError(
                f"Unsupported format_version {fv!r} (this build expects {BOOMERPACK_FORMAT_VERSION})"
            )
        meta: dict[str, Any] = {}
